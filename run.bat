...
for /d %%d in (*) do (
	...
	if exist "!fontmin_dir!" rd /s /q "!fontmin_dir!"
    md "!fontmin_dir!"
    ...
    for /f "delims=" %%f in ('dir /b /a-d "*.ttf"') do (
        ...
        echo ^@font-face{font-family:'%%d';src:url^('!filename!.woff2'^) format^('woff2'^);font-weight:!font_weight!;font-style:normal;font-display:swap;} >> %root_dir%web\!lower_dir!\!lower_dir!.css
